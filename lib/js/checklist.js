...
.controller('Controller', function() {
  ...
  checklist.getValidTasks = function(type){
    ...
    var questions = deepCopyArray(checklist.questions);
  ...
  checklist.getTypeByIndex = function(index){
    if (checklist.questionTypes[index].value) {
      return checklist.questionTypes[index].value;
    }
    return "";
  };
...
function getQuestionRequiredProperty(expectedBool){
  if (expectedBool){
    return 'questionsRequiredYes';
  }
  return 'questionsRequiredNo';
}
...
function filterTasksByRequired(tasks,questions,expectedBool){
  var newTasks = [];
  var property = getQuestionRequiredProperty(expectedBool);

  for (var i = 0; i < tasks.length; i++) {
    var addToTasks = true;
    for (var x = 0; x < tasks[i][property].length; x++) {
      questionAnswer = findQuestionAnswer(questions,tasks[i][property][x]);
      if (questionAnswer === !expectedBool || questionAnswer === null){
        addToTasks = false;
        break;
      }
    }
...
function filterTasksByAny(tasks,questions,expectedBool){
  var newTasks = [];
  var property = getQuestionsAnyProperty(expectedBool);
  ...
  for (var i = 0; i < tasks.length; i++) {
    ...
    for (var x = 0; x < tasks[i][property].length; x++) {
      var questionAnswer = findQuestionAnswer(questions,tasks[i][property][x]);
      if (questionAnswer === expectedBool){
        newTasks.push(tasks[i]);
        break;
      }
    }
...
function findQuestionAnswer(questions,questionId){
  for (var i = 0; i < questions.length; i++) {
    if (questions[i].id == questionId){
      return questions[i].default;
    }
  }
  return null;
}
...
function findRoleBasedOnProperty(roles,property,propertyValue){

  for (var i = 0; i < roles.length; i++) {
    if (roles[i][property] == propertyValue){
      return roles[i];
    }
  }
  return "";
...
function getOwnerProperty(roles,property,propertyValue,returnProperty){
  var role = findRoleBasedOnProperty(roles,property,propertyValue);
  if (!role[returnProperty]){
    return "";
  }
